...
router.put("/:id", async (req, res) => {
    ...
    if (programName !== undefined)
        update.programName = programName;
    if (bio !== undefined)
        update.bio = bio;
    ...
    const account = await UserAccountsModel.findByIdAndUpdate(req.params.id, update, { new: true }).select("-password");
    if (!account) {
...
router.post("/coach-invites", async (req, res) => {
...
router.get("/coach-invites/:token", async (req, res) => {
...
router.post("/coach-invites/:token/register", async (req, res) => {
    const invite = await CoachInviteModel.findOne({ token: req.params.token });
    if (!invite || invite.used || invite.expiresAt < new Date()) {
        res.status(404).json({ message: "Invite link is invalid or expired" });
        return;
    }
    const { fullName, phone, programName, password, bio, experience, languages, expertise, photo, availabilitySummary, maxWorkload, availabilityType, availableDays, } = req.body;
    if (!fullName) {
        res.status(400).json({ message: "Full name is required" });
        return;
    }
    const hashedPassword = password ? await bcrypt.hash(password, 10) : undefined;
    const derivedAvailabilitySummary = availabilitySummary || (availabilityType === "whole_week"
        ? "Whole week"
        : (availableDays && availableDays.length > 0 ? availableDays.join(", ") : "By discovery call"));
    const account = await UserAccountsModel.findOneAndUpdate({ email: invite.email }, {
        fullName,
        email: invite.email,
        phone,
        programName,
