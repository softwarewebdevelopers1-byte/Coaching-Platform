...
import { NotificationModel, ProgramModel, SessionNoteModel, TestimonialModel, AppNotificationModel, } from "../models/Platform.model.js";
...
router.post("/programs", async (req, res) => {
    const program = await ProgramModel.findOneAndUpdate({ slug: req.body.slug }, req.body, { new: true, upsert: true, setDefaultsOnInsert: true });
...
router.get("/session-notes", async (req, res) => {
    ...
    if (req.query.clientEmail)
        filter.clientEmail = String(req.query.clientEmail);
    const notes = await SessionNoteModel.find(filter).sort({ createdAt: -1 });
    res.status(200).json({ notes });
});
router.get("/analytics", async (_req, res) => {
    const [coaches, activeCoaches, bookings, openSlots, bookedSlots, notifications, contactLeads, newLeads, contactedLeads, scheduledLeads, convertedLeads, closedLeads, individualInterest, groupInterest, bothInterest,] = await Promise.all([
    ...
    const leadToBookingConversionRate = contactLeads > 0 ? Number((scheduledLeads / contactLeads).toFixed(2)) : 0;
    res.status(200).json({
        analytics: {
            coaches,
            activeCoaches,
...
router.patch("/app-notifications/:id/read", async (req, res) => {
    const notification = await AppNotificationModel.findByIdAndUpdate(req.params.id, { read: true }, { new: true });
    if (!notification) {
...
router.patch("/app-notifications/read-all", async (req, res) => {
    const { recipientId } = req.body;
    if (!recipientId) {
        res.status(400).json({ message: "recipientId is required" });
        return;
    }
    await AppNotificationModel.updateMany({ recipientId: String(recipientId), read: false }, { read: true });
