...
import { sendBookingConfirmationEmail, sendContactAcknowledgmentEmail, sendSlotRequestCoachNotificationEmail, sendSlotRequestReceivedEmail, } from "../services/Brevo.emailSender.js";
...
router.get("/", async (req, res) => {
    ...
    if (req.query.interest)
        filter.interest = String(req.query.interest);
...
router.patch("/:id", async (req, res) => {
    ...
    if (assignedCoachId !== undefined)
        update.assignedCoachId = assignedCoachId;
    if (assignedCoachName !== undefined)
        update.assignedCoachName = assignedCoachName;
    const submission = await ContactSubmissionModel.findByIdAndUpdate(req.params.id, update, { new: true });
    if (!submission) {
...
router.post("/:id/schedule", async (req, res) => {
    ...
    const { action = "book", programName, coachId, coachName, coachEmail, coachPhone, slotId, bookingTime, message, } = req.body;
    const resolvedProgram = programName || lead.programSlug || interestToProgramSlug(lead.interest);
    if (!resolvedProgram) {
        res.status(400).json({
            message: "Program is required for leads interested in both individual and group coaching",
        });
        return;
    }
    ...
    await ClientModel.findOneAndUpdate({ email: lead.email }, {
        fullName: lead.name,
        email: lead.email,
        phoneNumber: lead.phone,
        goals,
    ...
    const submission = await ContactSubmissionModel.findByIdAndUpdate(lead._id, {
        status: "scheduled",
