...
const splitProgramValues = (value) => value
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
const resolveProgram = (value) => PROGRAMS.find((program) => program.id === value ||
    normalize(program.title) === normalize(value) ||
    program.title === value);
export const programsMatch = (a, b) => {
    if (!a || !b)
        return false;
    const valuesA = splitProgramValues(a);
    const valuesB = splitProgramValues(b);
    if (valuesA.length > 1 || valuesB.length > 1) {
        return valuesA.some((valueA) => valuesB.some((valueB) => programsMatch(valueA, valueB)));
    }
    ...
    if (programA)
        return programA.id === b || normalize(programA.title) === normalize(b);
    if (programB)
        return programB.id === a || normalize(programB.title) === normalize(a);
    return false;
};
