...
export async function sendContactAcknowledgmentEmail(details) {
    const apiKey = DotEnvConfig.BrevoApiKey.trim();
...
export async function sendSlotRequestReceivedEmail(details) {
    const apiKey = DotEnvConfig.BrevoApiKey.trim();
    const currentYear = new Date().getFullYear();
    const htmlContent = `<!DOCTYPE html>
<html>
...
<body>
  <div class="email-container">
    ...
    try {
        const response = await axios.post("https://api.brevo.com/v3/smtp/email", payload, { headers: { "api-key": apiKey, "Content-Type": "application/json" } });
...
export async function sendSlotRequestApprovedEmail(details) {
    const apiKey = DotEnvConfig.BrevoApiKey.trim();
    const currentYear = new Date().getFullYear();
...
export async function sendSlotRequestDeclinedEmail(details) {
    const apiKey = DotEnvConfig.BrevoApiKey.trim();
    const currentYear = new Date().getFullYear();
    const htmlContent = `<!DOCTYPE html>
<html>
...
export async function sendSlotRequestCoachNotificationEmail(details) {
    const apiKey = DotEnvConfig.BrevoApiKey.trim();
    const currentYear = new Date().getFullYear();
...
function handleEmailError(error) {
    if (error.response) {
        console.error(`Email sending failed with status ${error.response.status}:`, error.response.data);
        const errorData = error.response.data;
        if (errorData.code === "unauthorized") {
            console.error("Invalid API key. Please check your BREVO_API_KEY environment variable.");
        }
        else if (errorData.code === "invalid_parameter") {
            console.error("Invalid email parameters. Please check the email format and content.");
        }
    }
    else if (error.request) {
        console.error("No response received from Brevo API. Please check your network connection.");
    }
    else {
        console.error("Error setting up email request:", error.message);
    }
}
//# sourceMappingURL=Brevo.emailSender.js.map